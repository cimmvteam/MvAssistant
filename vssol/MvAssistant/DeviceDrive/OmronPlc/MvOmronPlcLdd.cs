        ...
        public object Read(string VarName)
        {
            ...
            for (var tryIndex = 0; tryIndex < 3; tryIndex++)
            {
                try
                {
                    lock (this)
                    {
                        LogInfo(DateTime.Now.ToString("yyyyMMdd_HHmmss") + " Read VariableName = " + VarName);
                        //每個要存取PLC的 都要稍等一下, 讓PLC有恢復Clock的時間
                        Thread.Sleep(50);
                        LogInfo(DateTime.Now.ToString("yyyyMMdd_HHmmss") + " Write " + VarName + " = " + _CIPcompolet.ReadVariable(VarName) + " success");
                        return _CIPcompolet.ReadVariable(VarName);
                    }
                }
                catch (Exception ex)
                {
                    LogInfo(DateTime.Now.ToString("yyyyMMdd_HHmmss") + " Read " + VarName + " fail");
                    LogInfo(DateTime.Now.ToString("yyyyMMdd_HHmmss") + " Exception message : " + ex.Message);
        ...
        public void Write(string VarName, Object data)
        {
            ...
            for (var tryIndex = 0; tryIndex < 3; tryIndex++)
            {
                try
                {
                    lock (this)
                    {
                        LogInfo(DateTime.Now.ToString("yyyyMMdd_HHmmss") + " Write VariableName = " + VarName + " , Data = " + data);
                        //            //每個要存取PLC的 都要稍等一下, 讓PLC有恢復Clock的時間
                        //            Thread.Sleep(50);
                        this._CIPcompolet.WriteVariable(VarName, data);
                        LogInfo(DateTime.Now.ToString("yyyyMMdd_HHmmss") + " Write " + VarName + " success");
                        return;
                    }
                }
                catch (Exception ex)
                {
                    LogInfo(DateTime.Now.ToString("yyyyMMdd_HHmmss") + " Write " + VarName + " fail");
                    LogInfo(DateTime.Now.ToString("yyyyMMdd_HHmmss") + " Exception message : " + ex.Message);
                    MvLog.WarnNs(this, ex);
                    myex = ex;
                }
