...
namespace MvAssistant.v0_2
{
    public class MvaUtil
    {
        ...
        public static void DisposeTask(MvaTask task, int millisecond = 100)
        {
            if (task == null) return;
            if (task.Status < TaskStatus.RanToCompletion && millisecond >= 0) task.Wait(millisecond);
            task.Dispose();
        }
        public static void DisposeTask(MvaCancelTask task, int millisecond = 100)
        {
            if (task == null) return;
            if (task.Status < TaskStatus.RanToCompletion)
            {
                task.Cancel();
                if (millisecond >= 0) task.Wait(millisecond);
            }
            task.Dispose();
        }
